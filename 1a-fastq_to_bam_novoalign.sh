...
novoalign -d $LOCAL/*.fasta \
	-f $LOCAL/*$FASTQ1_SUFFIX $LOCAL/*$FASTQ2_SUFFIX \
	-o SAM "@RG\tID:$SAMPLE_NAME\tPL:$PLATFORM\tPU:"$FLOWCELL.$LANE"\tLB:$LIBRARY\tSM:$BARCODE" \
	-l 30 \
	-a ACACTGACGACATGGTTCTACA TACGGTAGCAGAGACTTGGTCT \
	-i PE 30-300 \
	-t 250 \
	-c $N_NODES \
	-n 80 \
	-k \
	-o FullNW \
	2>$REPORTS_ALIGNMENT_DIR/$SAMPLE_NAME.novostats.txt | \
samtools view -1 -bS - > $LOCAL/$SAMPLE_NAME.bam
# NOVOALIGN command description:
# -d reference genome
# -f input files
# -o output format and @RG annotation
# -l minimum number of matching bases
# -a adapter sequence for trimming
# -i paired end option
# -t alignment score threshold
# -c multithreading
# -n hard trimming
# -k base recalibration (on)
# -o FullNW soft clipping
# save novoalign report
...
novosort  -m $MEMORY \
	-t $LOCAL \
	-c $N_NODES \
	$LOCAL/$SAMPLE_NAME.bam \
	-i \
	-o $LOCAL/$SAMPLE_NAME.sorted.bam
...
# COPY SCRIPT
# scp /Users/callar01/OneDrive\ -\ Cancer\ Research\ UK\,\ Cambridge\ Institute/CRIdocs/ctDNA/pipeline_example/Scripts/1a-fastq_to_bam_novoalign.sh callar01@clust1-headnode.cri.camres.org:/Users/callar01/ctDNA/pipeline_example

# RUN JOBS
# sbatch --array=1-9%9 --job-name ngtas --mem 16000 -n 8 --output /scratchb/cclab/callar01/log/ngtas.%A.%a.out.txt --error /scratchb/cclab/callar01/log/ngtas.%A.%a.err.txt /Users/callar01/ctDNA/pipeline_example/1a-fastq_to_bam_novoalign.sh 
